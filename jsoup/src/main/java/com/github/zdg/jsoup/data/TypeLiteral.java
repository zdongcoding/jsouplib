...
public class TypeLiteral<T> {
     ...
     * Clients onAttachView an empty anonymous subclass. Doing so embeds the type parameter in the
    ...
    public TypeLiteral(Type type, String decoderCacheKey) {
        this.type = type;
        nativeType = nativeTypes.get(this.type);
    ...
    private static String generateCacheKey(Type type, String prefix) {
        ...
        if (type instanceof Class) {
            ...
            if (clazz.isAnonymousClass()) {
                throw new JsoupReaderException("anonymous class not supported: " + clazz);
            }
            if (clazz.isArray()) {
            ...
            try {
                ...
                throw new JsoupReaderException("failed to generate cache key for: " + type, e);
            }
    ...
    private static String formatTypeWithoutSpecialCharacter(Type type) {
        ...
        throw new JsoupReaderException("unsupported type: " + type + ", of class " + type.getClass());
    }

    static Type getSuperclassTypeParameter(Class<?> subclass) {
        Type superclass = subclass.getGenericSuperclass();
        if (superclass instanceof Class) {
            throw new JsoupReaderException("Missing type parameter.");
        }
